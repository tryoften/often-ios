...
class MediaItem: Equatable {
    ...
    class func mediaItemFromType(_ data: NSDictionary) -> MediaItem? {
     ...
     */
    class func modelsFromDictionaryArray(_ array: NSArray) -> [MediaItem] {
    ...
    required init(data: NSDictionary) {
        ...
        if let time_added = data["time_added"] as? Double {
            let date = Date(timeIntervalSince1970: time_added / 1000.0)
            self.created = date
        }
        ...
        if let image = data[imageProperty] as? String, let imageURL = URL(string: image) {
        ...
        if let images = data["images"] as? NSDictionary,
            ...
            let image_url = small["url"] as? String {
                self.smallImageURL = URL(string: image_url)
        }
        ...
        if let images = data["images"] as? NSDictionary,
            ...
            let image_url = small["url"] as? String {
                self.squareImageURL = URL(string: image_url)
        }
        ...
        if let images = data["images"] as? NSDictionary,
            ...
            let image_url = medium["url"] as? String {
                self.mediumImageURL = URL(string: image_url)
        }
        ...
        if let images = data["images"] as? NSDictionary,
            ...
            let image_url = large["url"] as? String {
                self.largeImageURL = URL(string: image_url)
        }
